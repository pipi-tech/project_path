#!/bin/bash
# WHAT:  Silent dependency watcher — uses inotifywait to detect pip installs without explicit commands
# WIRES: Called by functions/logs.sh (watchDeps) → writes to data/log/dep/input/silent/
# WHY:   Catches dep changes that happen outside the dep_wrappers intercept layer
