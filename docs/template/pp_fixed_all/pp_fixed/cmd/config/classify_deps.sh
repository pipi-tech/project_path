#!/bin/bash
# WHAT:  Classifies installed packages into categories (nlp/audio/vision/etc) using pkg_classes.conf
# WIRES: Called by functions/config.sh (classify) → reads project.toml and pkg_classes.conf → writes categorized output
# WHY:   Dep visibility — helps understand what a project depends on at a semantic level
