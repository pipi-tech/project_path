#!/bin/bash
# WHAT:  Migrates or creates a project.toml from current pip state and auto-detects managers
# WIRES: Called by functions/config.sh (migrate) → reads current venv → writes project.toml → updates registry.conf
# WHY:   Onboarding tool — bootstraps a toml for projects that don't have one yet
