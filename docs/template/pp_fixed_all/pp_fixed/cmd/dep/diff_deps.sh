#!/bin/bash
# WHAT:  Diffs before/after pip snapshots to find what was added or removed
# WIRES: Called by functions/dep_wrappers.sh (_dep_wrap) → reads data/log/dep/input/before+after/ → writes to diff/
# WHY:   Makes dep changes explicit and auditable — also syncs project.toml requires field
