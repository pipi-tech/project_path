#!/bin/bash
# WHAT:  Captures pip state snapshot after a dep install
# WIRES: Called by functions/dep_wrappers.sh (_dep_wrap) → writes to data/log/dep/input/after/
# WHY:   Post-install state so diff_deps can compute the exact change
