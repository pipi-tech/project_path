...
if ! grep -q "^$PROJECT_NAME:" "$REG_CONF" 2>/dev/null; then
  echo "$PROJECT_NAME:$HOME/project/$PROJECT_NAME:$PROJECT_VENV:$PROJECT_ENV:$PROJECT_TYPE" >> "$REG_CONF"
  bash "$PTH_ROOT/data/log/registry/src/on_register.sh" "$PROJECT_NAME" "$TS"
else
  sed -i "s|^$PROJECT_NAME:.*|$PROJECT_NAME:$HOME/project/$PROJECT_NAME:$PROJECT_VENV:$PROJECT_ENV:$PROJECT_TYPE|" "$REG_CONF"
