#!/bin/bash
# WHAT:  Event router for the doc_worker — dispatches build_done, dep_install, session_open, session_close events
# WIRES: Called by functions/proj.sh and functions/doc.sh → dispatches to gen_ref, watch_deps, session_open, session_close
# WHY:   Single entry point for all doc_worker events — keeps event routing in one place
