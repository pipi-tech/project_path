#!/bin/bash
# WHAT:  Fills in closed timestamp and duration in the open session log file
# WIRES: Called by engines/doc_worker/src/main.sh (session_close event) → edits session file in project_root/docs/log/
# WHY:   Closes the session record — without this, sessions have no end time or duration
