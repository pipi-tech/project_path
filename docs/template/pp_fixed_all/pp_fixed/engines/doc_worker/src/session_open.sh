#!/bin/bash
# WHAT:  Creates a new session log file for a project when it is entered
# WIRES: Called by engines/doc_worker/src/main.sh (session_open event) → writes to project_root/docs/log/session_TS.txt
# WHY:   Session boundary marker — every proj entry creates a traceable session record
