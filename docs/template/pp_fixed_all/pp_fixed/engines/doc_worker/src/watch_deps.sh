#!/bin/bash
# WHAT:  Appends a dep install event to the project's changelog.txt
# WIRES: Called by engines/doc_worker/src/main.sh (dep_install event) → writes to project_root/docs/log/changelog.txt
# WHY:   Dep history — every install gets a timestamped entry so you can trace when deps were added
